...
pub async fn connect_ftp(
    ...
    let mut connect_key = Uuid::new_v4().to_string();
    if key.len() < 1 {
        connect_key = key.to_string();
    }
    FTP_CLIENTS
        .lock()
        .map_err(|_| "锁获取失败".to_string())?
        .insert(connect_key.to_string(), ftp);
    Ok(connect_key)
}
...
#[tauri::command]
pub async fn ftp_list_dir(key: &str, path: &str) -> Result<Vec<String>, String> {
    let mut client = FTP_CLIENTS.lock().map_err(|_| "锁获取失败".to_string())?;
    let ftp = client
        .get_mut(key)
        .ok_or_else(|| "指定的FTP连接不存在".to_string())?;
...
#[tauri::command]
pub async fn ftp_create_dir(key: &str, path: &str) -> Result<(), String> {
    let mut client = FTP_CLIENTS.lock().map_err(|_| "锁获取失败".to_string())?;
    let ftp = client
        .get_mut(key)
        .ok_or_else(|| "指定的FTP连接不存在".to_string())?;
    ftp.mkdir(&format!("{}", path))
